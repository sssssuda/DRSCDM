...
class DataPoint1:
    def __init__(self, index, embed, kernel=None, shell=None, membership=None):
        if kernel is None:
            kernel = set()
        if shell is None:
            shell = set()
...
class Cluster1:
    def __init__(self, name, p: DataPoint1):
        self.point = set()
        self.name = name

        self.point.add(p)

    def add(self, q: DataPoint1):
        self.point.add(q)
...
class BaseClustering2:
    ...
    def add(self, p: DataPoint2):
        if self.clusters is None:
            cluster = {"name": "c1", "embeds": p.embed, "centroid": p.embed, "timeline": p.index}
        ...
        if sim_matrix[0][max_index - 1] >= self.thrs:
            ...
            for i, cluster in enumerate(self.clusters, start=1):
                if i == max_index:
                    ...
                    cluster["timeline"] += p.index
        else:
            cluster = {"name": f"c{len(self.clusters) + 1}", "embeds": p.embed,
                       "centroid": p.embed, "timeline": p.index}
    ...
    def result(self):
        ...
        for p in self.pList:
            ...
            if label not in result:
                ...
                result[label] = result[label].union(p.timeline)

            self.res = result

        if hold_list:
            cname = f"c{len(result) + 1}"
            self.res[cname] = hold_list[0].timeline
            for p in hold_list[1:]:
                self.res[cname] = self.res[cname].union(p.timeline)
